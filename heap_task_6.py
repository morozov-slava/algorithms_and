class Heap:
    ...
    # (бонус +500) Добавьте метод объединения текущей кучи с кучей-параметром. Используйте для этого только внешний интерфейс класса (без доступа/преобразования в массив и обратно). 
    # Подумайте, как сделать это с минимальными затратами по времени. Убедитесь, что после объединения новое дерево сохраняет свойства кучи.
    def Combine(self, heap: Heap):
        self.HeapSize += len(heap.HeapArray)
