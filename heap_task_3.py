class Heap:
    ...
    def IsCorrectHeap(self):
        if len(self.HeapArray) == 0:
            return True
        return self._is_correct_heap(0)

    def _is_correct_heap(self, i: int):
        if i >= len(self.HeapArray):
            return True
        lc_i = 2*i + 1
        rc_i = 2*i + 2
        if (lc_i < len(self.HeapArray)) and (self.HeapArray[i] < self.HeapArray[lc_i]):
            return False
        if (rc_i < len(self.HeapArray)) and (self.HeapArray[i] < self.HeapArray[rc_i]):
            return False
        left_subtree = self._is_correct_heap(lc_i)
        right_subtree = self._is_correct_heap(rc_i)
        return all([left_subtree, right_subtree])    
